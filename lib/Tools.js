...
const Tools = {
   ...
   */
   ...
   ex_length(string, redundancyLv = 0) {
      ...
      let emojiUnitLength = 0;
      for (const emoji of emojis) emojiUnitLength += emoji.length;

      return emojis.length * 2 + (r1_length - emojiUnitLength);
   },
      ...
      getTimeFromMS(ms) {
         ...
         if (useDDMMYYYFormat) {
            return {
               ...
               modern() {
                  if (parsedTime.year + parsedTime.month + parsedTime.days + parsedTime.sec + parsedTime.min + parsedTime.hr == 0)
                     return `${Tools.maxFraction(parsedTime.ms, 4)}ms`;
                  else if (parsedTime.year + parsedTime.month + parsedTime.days + parsedTime.min + parsedTime.hr == 0)
                     return `${parsedTime.sec}.${Math.round(parsedTime.ms).toString().padStart(3, '0')}s`;
                  else if (parsedTime.year + parsedTime.month + parsedTime.days + parsedTime.hr == 0) return `${parsedTime.min}min ${parsedTime.sec}s`;
                  else if (parsedTime.year + parsedTime.month + parsedTime.days == 0) return `${parsedTime.hr}hr ${parsedTime.min}min`;
                  else if (parsedTime.year + parsedTime.month == 0) return `${parsedTime.days}days ${parsedTime.hr}hr`;
                  else if (parsedTime.year == 0) return `${parsedTime.month}months ${parsedTime.days}days`;
         ...
         return {
            ...
            modern() {
               if (parsedTime.sec + parsedTime.min + parsedTime.hr == 0) return `${Tools.maxFraction(parsedTime.ms, 4)}ms`;
   ...
   /**
    * @typedef {'Reset'|'Bright'|'Dim'|'Italic'|'Blink'|'Invert'|'Underline'|'Normal'|'Hidden'|'Black'|'Red'|'Green'|'Yellow'|'Blue'|'Magenta'|'Cyan'|'White'|'BgBlack'|'BgRed'|'BgGreen'|'BgYellow'|'BgBlue'|'BgMagenta'|'BgCyan'|'BgWhite'|number} NCCColorOptions
    */
   ...
   strWrap(str, maxLineLength, options = {}) {
      ...
      for (let i = 0; i < lines.length; i++) {
         while (Tools.ex_length(lines[i], redundancyLv) > maxLineLength) {
            ...
            content.push(lines[i].substring(0, safeWrapIndex).concat('\n'));
         ...
         content.push(lines[i].concat(i < lines.length - 1 ? '\n' : ''));
      }
      return content.join('');
      ...
      function getWrapIndex(line) {
         ...
         if (!matchSoftOnly) return fallbackIndex || totalRawLength || line.length;
         const target =
            lastSoftAfterInner ??
            lastHardAfterInner ??
            firstSoftAfterMax ??
            ...
            lastSoftBeforeMax ??
            lastHardBeforeMax;
         if (target) return target.breakIndex;
         return fallbackIndex || totalRawLength || line.length;
         ...
         function processPiece(piece, isAnsi) {
            ...
            if (!isAnsi) {
               if (softSep.test(piece)) {
                  ...
                  const record = { breakIndex, width: breakWidth };
            ...
            if (lastSoftAfterInner || lastHardAfterInner) return true;
