   ...
   */
   ...
   REGEXP: Object.freeze({
      ...
      */
      ...
      HardWrapSeperators: /(?<!\x1b(?:\[[0-9;]*m|\]8;;[^\x07\x1b]*(?:\x07|\x1b\\)))[-\s_・⧸/\\;:()\[\]、,.'＂"!^*＊=+「【】」（）]/g,
      /**
   ...
   strWrap(str, maxLineLength, options = {}) {
      ...
      for (let i = 0; i < lines.length; i++) {
         const allIndex = Tools.getMatchAllIndexes(lines[i].matchAll(SoftSep_reg));
         ...
         while (Tools.ex_length(lines[i], redundancyLv) > maxLineLength) {
            ...
            if (mode == 'softboundary') {
               if (indexOfSep == -1) {
                  const indexesOfHardSep = Tools.getMatchAllIndexes(lines[i].matchAll(HardSep_reg))
                     .filter(v => v >= innerBound);
            ...
            content += lines[i].substring(0, indexOfSep).concat('\n');
