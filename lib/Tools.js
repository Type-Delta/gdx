   ...
   */
   ...
   async asyncSleep(milliseconds) {
      ...
      return new Promise(resolve => {
         if (!milliseconds || milliseconds < MIN_SLEEP_MS || typeof milliseconds != 'number') resolve();
      ...
      [`exit`, `SIGINT`, `SIGUSR1`, `SIGUSR2`, `uncaughtException`, `SIGTERM`].forEach((eventType) => {
         process.on(eventType, (...args) => {
            ...
            if (res instanceof Promise) {
               res.then(() => {
                  if (eventType !== 'exit' && preventImmediateExit)
                     process.exit(0);
               }).catch((err) => {
   ...
   compareDate(dateA, operand, dateB, resolution = 'day') {
      ...
      let compA = dateAGen.next(), compB = dateBGen.next();
      do {
         if (compA.value !== compB.value) {
            switch (operand) {
               case '>': return compA.value > compB.value;
               case '<': return compA.value < compB.value;
               case '>=': return compA.value >= compB.value;
               case '<=': return compA.value <= compB.value;
    ...
    */
   deferredFunc(
      callback, delay = 200, immediate = false
   ) {
      let timeout = null;
      ...
      getTimeFromMS(ms) {
         ...
         if (useDDMMYYYFormat) {
            return {
               ...
               modern() {
                  if (year + month + days + sec + min + hr == 0) return `${Tools.maxFraction(ms, 4)}ms`;
                  else if (year + month + days + min + hr == 0) return `${sec}.${Math.round(ms).toString().padStart(3, '0')}s`
         ...
         return {
            year, month, days, hr, min, sec, ms,
            toString() { return `${hr}:${min}:${sec}.${Math.round(ms).toString().padStart(3, '0')}`; },
            modern() {
               if (sec + min + hr == 0) return `${Tools.maxFraction(ms, 4)}ms`;
               else if (min + hr == 0) return `${sec}.${Math.round(ms).toString().padStart(3, '0')}s`
   ...
   maxFraction(number, maxFractDigits, floor = false) {
      ...
      number = (
         floor ? Math.floor(number * factor) : Math.round(number * factor)
      ) / factor;
   ...
   objToString(obj, options = {}) {
      ...
      if (!noHeader && (isNativeClass || keyCount > 5)) {
         if (color)
            header = nccColor.magenta + `${className}(${keyCount}) ` + nccColor._default;
         else
            header = `${className}(${keyCount}) `;
      }
      ...
      function traverse(obj, depth, indentCount = 0) {
         ...
         // Handle array-like
         if (Tools.isArrayLike(obj)) {
            return Tools.arrToString(obj, { color, defColor, maxDepth: maxDepth - depth, indent, noHeader: true });
         }
         ...
         for (let i = 0; i < keysToShow; i++) {
            ...
            // Value handling
            if (value && typeof value === 'object' && !Tools.isArrayLike(value) &&
   ...
   REGEXP: Object.freeze({
      ...
      */
       ...
       * this will ignore any ANSI code
      */
      HardWrapSeperators: /\x1b(?:\[[0-9;]*m|\]8;;[^\x07\x1b]*(?:\x07|\x1b\\))|([-_・⧸/\\、'＂"!^*＊=+「【】」（）])/g,
      /**
         ...
         if (keyIndex == -1) {
            ...
            if (useIniGroup) {
               if (cantFindGroup) {
                  ...
                  const lastPairIndex = Tools.redexOf(savedConfig, new RegExp(`^\\s*\n$`, 'm'), existGroupIndex);
   ...
   strSlice(str, start, end, redundancyLv = 0) {
      let ANSIIndexes = [...str.matchAll(Tools.REGEXP.ANSICode)]
         .map(v => {
   ...
   strWrap(str, maxLineLength, options = {}) {
      ...
      for (let i = 0; i < lines.length; i++) {
         const allIndex = [];
         for (const match of lines[i].matchAll(SoftSep_reg)) {
            if (!match[1]) continue;
            allIndex.push(match.index);
         }

         // TODO: in this while(), change to: slice cursor to cursor + maxLineLength and search for the last seperator in that range, if not found, insert line break at cursor + maxLineLength, then move by maxLineLength and repeat until the end of the line is reached
         while (Tools.ex_length(lines[i], redundancyLv) > maxLineLength) {
            const indexesOfSep = [];
            for (let j = 0; j < allIndex.length; j++) {
               const index = allIndex[j] - indexOffset;
               if (index >= innerBound) indexesOfSep.push(index);
            }
            ...
            if (matchSoftOnly) {
               if (indexOfSep == -1) {
                  const indexesOfHardSep = [];
                  for (const match of lines[i].matchAll(HardSep_reg)) {
                     if (!match[1] || match.index < innerBound) continue;
                     indexesOfHardSep.push(match.index);
                  }
               ...
               indexOfSep = (
                  (indexOfSep > maxLineLength) || (indexOfSep == -1)
                     ? maxLineLength
                     : indexOfSep
               );
            }
            ...
            content.push(lines[i].substring(0, indexOfSep).concat('\n'));
            lines[i] = lines[i].substring(indexOfSep).trimStart();
      ...
      function getSepIndex(sepIndexes, index) {
         for (let i = index; i < sepIndexes.length; i++) {
            if (!sepIndexes[i + 1]) return sepIndexes[i];
            if (sepIndexes[i + 1] - sepIndexes[i] > 1) return sepIndexes[i];
         }

         return sepIndexes[sepIndexes.length - 1];
      }
   },
    ...
    */
   toShortNum(number, maxFractDigits = 2, k = 1e3, floor = false, minPower = -Number.MAX_SAFE_INTEGER, maxPower = Number.MAX_SAFE_INTEGER) {
      ...
      const i = Tools.MathKit.clamp(
         Math.floor(Math.log(absNum) / Math.log(k)),
         minPower,
         maxPower
      );
   ...
   yuString(whatever, options = {}) {
      ...
      switch (typeof whatever) {
         case 'string':
            if (color)
               return Tools.ncc(0x7fc44a) + `"${whatever}"` + Tools.ncc(defColor);
            return `"${whatever}"`;
         case 'bigint':
         case 'number':
            if (color)
               return Tools.ncc(0xb12591) + whatever + Tools.ncc(defColor);
         // eslint-disable-next-line no-fallthrough
         case 'boolean':
         case 'undefined':
            if (color)
               return Tools.ncc(0xf06ba2) + whatever + Tools.ncc(defColor);
            return whatever + '';
         case 'function':
      ...
      if (whatever instanceof Date) {
         if (color)
            return Tools.ncc(0xb03fba) + 'Date' + Tools.ncc(defColor) + ' ' + whatever.toISOString();
         return 'Date ' + whatever.toISOString();
      }

      if (whatever instanceof RegExp) {
         if (color)
            return Tools.ncc('Yellow') + whatever.toString() + Tools.ncc(defColor);
         return whatever.toString();
      }
